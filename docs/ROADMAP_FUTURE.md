...
| **GAL-1a** | **Avatars in the gallery** *(added 2026-07-12)* | Surface partner/profile avatars (the `photoMediaId` blobs with no `media` row) in the gallery — most naturally as the header image of each partner section in the by-partner layout. Deferred from GAL-1 because avatars carry no date and aren't filterable, so they need separate handling from encounter photos. |
