...
| **QOL-3** | **Partner editor → its own page** | ✅ **DONE (2026-07-29, `2fde28b`).** New `PartnerEditScreen` + `PartnerEditViewModel` (Hilt, SavedStateHandle) at nav route `partner/{partnerId}` (or `new`); old `PartnerDialog` deleted. Reuses `DemographicFields`/`OptionalChips`; carries the discard-changes guard (D-33). Profile editor uses the same shared `PersonPhotoStrip` — the "you" vs "a partner" surfaces edit identically. |
| **QOL-7** | **Promote remaining pop-out dialogs to their own pages** *(added 2026-07-12)* | ✅ **DONE (2026-08-31, v0.5.5).** Audit report + 2 conversions. Full grep found 15 UI files with dialog/sheet usage; every dialog is a proper picker (Date/Time/DateRange), a confirm gate (discard/delete), a short menu (photo action, More…), a filters sheet by design (Search + Gallery), or the WhatsNew post-update popup — all correctly stay. Only two substantial multi-field forms qualified: **CsvMappingDialog** → new `CsvImportScreen` route + `Routes.CSV_IMPORT` (removes the 460dp height cap, adds a detected-rows summary + format hint; file picker now lives on the route itself), and **ReassignDialog** → `ReassignPicker` full-screen `Dialog(usePlatformDefaultWidth = false)` with `TopAppBar` + search field (accent-folded via `EncounterSearch.fold`, matches date + partner names) — a big log no longer forces a long scroll. Discard-changes guard (D-33) intentionally not added: CSV import has no destructive save (rows are inserted only on Import), and reassign is a single tap → apply. See D-57. |
| **QOL-4** | **Updated icon set** | Launcher icon + per-act vector icons (`ic_act_*.xml`; set trimmed to 11 in v10 with the catalog). The current launcher is still the M1 "placeholder mark" (a simple flame in the adaptive-icon safe zone, `ic_launcher_foreground.xml` — self-labeled "Replace at M8"), and the F-Droid store `icon.png` is derived from that same vector (**STORE-2**). | New launcher icon (adaptive + **themed/monochrome** for Material You), refreshed act icons. Largely a design/drawable swap — act icons are already a drawable-swap-only seam (`ActVisuals`). Design brief: [design/ICON_PROJECT_PROMPT.md](../design/ICON_PROJECT_PROMPT.md). **Ship together with STORE-2** so the on-device launcher + F-Droid listing icon match. Can land in any release. |
...
| **QOL-6** | **Tooltips / first-run coach marks** *(added 2026-07-12)* | Some controls are icon-only (the gallery's search + filter buttons, the calendar toggle, chart-style pickers) with no label — discoverable by trial. | Two complementary, standard Android patterns: (1) **Material 3 tooltips** — wrap icon buttons in `TooltipBox` with a `PlainTooltip` (shows on **long-press**, and on hover for tablet/desktop/stylus); pure a11y-friendly labels, no state, no schema. (2) **First-run coach marks** — a one-time spotlight/callout overlay pointing at a new feature (e.g. the Photos tab, the filter button), dismissed forever after first view via a `tryst_general`-style "seen" flag (like the What's-new popup gate). Keep it **local-only** and skippable; never block the UI. Start with tooltips (cheap, high-value); add coach marks only for genuinely new surfaces. Fits the no-network/local-only invariants trivially. |
...
| **SRCH-2** | **"More filters" sheet** (expose the rest of FILT-1 in Search) | ✅ **DONE (2026-07-11, unreleased on `main`).** A badged "Filters" chip opens a `ModalBottomSheet` (`ui/search/MoreFiltersSheet`) surfacing every FILT-1 dimension the base chips don't: acts/positions/kinks/toys/occasions (user-catalog chips, `custom:`-prefixed; empty categories hide their section), place/protection/mood/initiator/weekday/time-of-day, a duration `RangeSlider`, has-note tri-state, and include-solo. **Live-apply** — each toggle updates results immediately; the pinned bar shows a running "Show N results" and "Reset" clears only the advanced dims. VM merges the advanced fields via a single `_advanced` holder + 2-arg `combine` (sidesteps `combine`'s 5-flow arity). No schema change; all static gates green + verified on the Pixel emulator (855 → 19 → reset → 855). See D-49. |
...
> **Video items moved out.** MED-1 (video attachments) and GAL-2 (gallery includes video) now
> live in their own file — **[ROADMAP_VIDEO.md](ROADMAP_VIDEO.md)** — because video is the
> first feature that needs a runtime dep (Media3 / ExoPlayer) and materially changes backup
> size. Photos and video are shipped as separate themes rather than one bundle.

| ID | Item | Proposed |
|----|------|----------|
| **GAL-1** | **Gallery by person / date** | ✅ **DONE (2026-07-12, unreleased on `main`).** A 5th **Photos** nav tab: `GalleryPhotos.build` (pure, JVM-tested) derives the gallery from `EncounterRepository.observeAll()` via the same `EncounterFilter` + `EncounterSearch` as Search — FILT-1's **third consumer**. **Four user-selectable layouts** (date grid / flat grid / by-partner / feed) + column density (2–4) + sort, persisted in `GalleryPreferences` (excluded from backup, default date grid), picked from the gallery's app-bar options sheet. Reuses the SRCH-2 "More filters" sheet + date/rating/partner chips + search. Full-screen `PhotoViewer` (HorizontalPager, pinch-zoom, double-tap, **Open tryst**). Decrypt-in-memory via `MediaImages`/`DecodedImage`; `FLAG_SECURE` protects it. **No schema change.** See D-50. **Scope:** encounter photos only — avatars deferred (**GAL-1a**). *Perf: `LazyVerticalGrid` composes visible tiles only; per-tile re-decode-on-scroll acceptable for v1, an LRU `ImageBitmap` cache keyed by `media.id` is the fast-follow.* |
| **GAL-1a** | **Avatars in the gallery** *(added 2026-07-12; partner-header avatars ✅ 2026-07-12; People layout ✅ 2026-07-29)* | ✅ **DONE.** Partner-section headers show the partner's avatar + name (2026-07-12). **People layout** (2026-07-29): a 5th `GalleryLayout.PEOPLE` renders partner + self-profile avatars as a browsable circular-avatar grid (from `PartnerEntity`/`ProfileEntity.photoMediaId`), separate from the encounter-photo pipeline (`GalleryPhotos.build` yields nothing for it — they carry no date and aren't filterable). Tapping opens a full-screen swipeable `AvatarViewer` across everyone. Empty-state when no avatars exist. |
| **GAL-1b** | **Justified/mosaic tiles** *(added 2026-07-12)* | ✅ **DONE (2026-07-29).** `GalleryLayout.MOSAIC` — aspect-preserving justified rows (Google-Photos style) under the same month headers as the date grid. Per-photo aspect ratios are decoded (bounds-only) and cached in the VM (`GalleryViewModel.aspectRatio`, `ConcurrentHashMap`), defaulting square until known so the grid settles as they load. Row packing is a pure `justify()` fn (target row height + clamp on extreme aspects); the `LazyColumn` stays lazy per row. |
| **META-1** | **Photo metadata** *(added 2026-07-12; display ✅ 2026-07-12)* | ✅ **DONE as display-only.** The photo viewer has an info (ⓘ) button showing embedded **capture date/time, dimensions, camera, and GPS coords** (raw — no `Geocoder`, which could hit the network), read at view time via `androidx.exifinterface` from the decrypted stream (`data/media/PhotoMetadata`) — **no schema change, nothing persisted.** **Deliberately does NOT strip EXIF on import** (user decision): EXIF is tiny (negligible storage), stripping means a lossy re-encode, and the app has no network / no share / encrypted-at-rest, so embedded location never leaves the device. **Future hook:** *if* a "share/export a single photo" feature is ever added, strip-on-export then. Original facets for reference below. — **(a) Privacy hygiene — strip/normalize EXIF on import.** Imported photos can carry EXIF **GPS coordinates, device model, and original timestamp**; today `EncounterRepository.attachMedia` encrypts the raw bytes as-is, so that metadata lives (encrypted) inside the blob and inside any backup. It's **not a live leak** (no network, encrypted at rest, no share feature yet), but scrubbing GPS/device EXIF in the import pipeline **before** encryption is cheap defense-in-depth — and matters the moment an export/share path exists. Re-encode or strip EXIF, keep orientation. **(b) Utility — surface + use capture metadata.** Read the photo's real **capture date/time + dimensions** (EXIF / `MediaMetadataRetriever`); show them in the `PhotoViewer`, and *optionally* let the gallery group/sort by the photo's **true taken-date** rather than the tryst's date (that needs a per-media `capturedAt` column → **schema change**; the tryst-date default is fine without it). Facet (a) and the viewer part of (b) need **no schema change**. |

#### Gallery v2 batch *(added 2026-07-29 — favourites, multi-select, and gallery UX cheap wins)*
| ID | Item | Status |
|----|------|--------|
| **GAL-3** | **Favourite photos** | ✅ **DONE (2026-07-29).** A per-photo favourite mark: `media.favorite` column (**schema v14 / `MIGRATION_13_14`**, additive `INTEGER NOT NULL DEFAULT 0`; `migrate13To14` test). It **round-trips through backup for free** — `BackupManager` dumps/restores `media` with a generic `SELECT *`, so no export-format change. Toggle from the viewer's heart; a "favourites only" heart toggle in the gallery app bar (`GalleryPhotos.build(onlyFavorites)`, JVM-tested); a heart badge on favourited tiles. |
| **GAL-4** | **Multi-select + bulk actions** | ✅ **DONE (2026-07-29).** Long-press a tile → selection mode (grid/feed/mosaic share `SelectablePhotoTile` + `TileInteraction`; check overlay + dim). Contextual `SelectionBar` with **bulk favourite / unfavourite / delete (confirm) / reassign-to-another-tryst**. New `MediaDao.setFavorite(ids)`, `reassign(ids, encounterId)`; repo `deleteMedia(list)`/`reassignMedia`. `BackHandler` exits selection first. |
| **GAL-6** | **Portrait album per person** *(added 2026-07-29)* | ✅ **DONE (2026-07-29, `2fde28b`).** New `person_photo` table (**schema v15 / `MIGRATION_14_15`**, additive) holds a per-person album beyond the single active avatar. Shared `PersonPhotoStrip` (in both the partner editor and the profile editor) with a "+" tile → camera / multi-pick / single-pick; existing portraits render in a LazyRow with a checkmark on the current avatar; tap → Set-as-avatar / Delete. `GalleryPhotos.build` folds portraits into the same sort/group/filter pipeline as encounter photos. Photo actions auto-save so a discard-changes only rolls back the form fields, never the album. Round-trips through backup (blob ids gathered from `person_photo` too). |
| **GAL-7** | **Solo tryst photos attribute to self + "Add to person's photos" from viewer** *(added 2026-07-30)* | ✅ **DONE (2026-07-30, `716bb23`).** Two related person-photo threads (D-52). (a) `GalleryPhotos.encounterPhoto` attributes a solo tryst's photos to `SELF_PARTNER_ID` (was: a nameless "Solo" bucket). By-partner puts them under You alongside the self-portrait album; drilling into self from People translates the filter to `includeSolo = true, partnerIds = {}` so `EncounterFilter.matches` still lets solo encounters through. (b) New `AddPhotoAlternate` action in `PhotoViewer` opens a menu of every partner + You; picking one copies the current photo into that person's `person_photo` album (source photo untouched). Cheap way to fix a mis-attribution or share a shot across multiple people. No schema change. |
| **GAL-5** | **Gallery UX cheap wins** | ✅ **DONE (2026-07-29).** **Pinch-to-zoom density** (2-finger pinch on the grid steps the persisted column count without eating single-finger scroll); **Partners → gallery deep link** (a "View photos" action opens Photos pre-filtered to that partner via a `GalleryDeepLink` singleton, mirroring `GalleryRevealState`); **slideshow** (viewer auto-advances every 3s, wrapping); **set-as-partner-avatar** (viewer copies the photo's bytes into a partner's avatar blob, swapping the old one); **filmstrip** (thumbnail strip in the viewer to jump within the set). |

**Deferred photo ideas (roadmap for future sessions — the "rest" per the 2026-07-29 request):**
| ID | Item | Notes |
|----|------|-------|
| **CAP-1** | **Per-photo captions** | ✅ **DONE (2026-08-31, v0.5.3).** Additive `media.caption TEXT NULL` (**schema v16 / `MIGRATION_15_16`**, `migrate15To16` test). Editable from the photo viewer, with a Settings picker for the entry point (**Settings → Gallery → Photo captions**: Info panel / Top toolbar / Both / Off, default Both — see D-55). Folded into the SRCH-1 index as `SearchField.PHOTO_CAPTION`; caption phrases surface the tryst they belong to. Round-trips through the encrypted backup for free (generic `SELECT *` dump). **Tile long-caption ✅ (2026-08-31, v0.5.6)** — grid + by-partner layouts show the caption as a second line under the tile when "Show date · partner under tiles" is on, and the Feed layout shows up to two lines below the date row. |
| **EDIT-1** | **In-app photo edit (crop / rotate)** | ✅ **DONE (2026-08-31, v0.5.4).** Photo viewer's new pencil action opens an Edit sheet: **Rotate 90° left/right** (applies immediately) and **Crop…** (full-screen editor with draggable rect, corner handles, and Free / 1 : 1 / 4 : 3 / 16 : 9 aspect chips). Decrypt → `Bitmap`/`Matrix` transform → JPEG q92 → re-encrypt in place (same blob id) via `EncryptedMediaStore.saveStaged` + `promoteStaged`, so a mid-edit failure leaves the live blob untouched. Downstream caches (`DecodedImage`, aspect map) invalidate via `GalleryViewModel.photoBustKey`. EXIF is dropped in the re-encode — the natural strip-on-edit safeguard D-51 reserved (D-56). No undo; edits replace. |
| **MEM-1** | **"On this day" memories** | A resurfacing card (gallery or Trysts tab) showing photos from this date in past years — fully local/derivable. **Flag-and-decide:** it *proactively* surfaces sensitive content, so it must respect the SEC-2 blur gate and ship **opt-in, off by default**. Threat-model note before building. |
| **EXP-1** | **Single-photo export** | Save a decrypted copy of one photo via SAF (no share intent). This is the **first deliberate "data leaves the vault" path**, so it's a threat-model conversation first, must fire META-1's **strip-EXIF-on-export** hook, and needs a clear warning. Deferred until that's decided. |
...
| **STORE-1** | **F-Droid listing screenshots** *(shipped 2026-07-02b, revised 2026-08-07 per issue #3; PULLED 2026-08-19 pending redesign)* | Was: 6 dark-theme screenshots shipped 2026-07-02b, trimmed to 5 on 2026-08-07 after F-Droid issue #3 flagged two as too explicit for the all-ages store page. **Pulled entirely 2026-08-19** — user judged the current captures aren't ready and wants a full redesign pairing with QOL-4's icon refresh. Store gallery is now empty until we reship. | Re-capture screenshots after the QOL-4 icon set + STORE-2 store icon land, using the same scrubbed-synthetic-dataset + `FLAG_SECURE`-off pipeline (helpers `tools/dataset/Dump.java`/`Extract.java`). All-ages compliant per issue #3 conversation. Optional `featureGraphic` also fair game this pass. | **LOW.** Design-bound; pairs with QOL-4 + STORE-2 as one shipment. |
| **STORE-2** | **F-Droid `icon.png`** *(added 2026-08-07 per F-Droid issue #3; placeholder PULLED 2026-08-19)* | A 512×512 PowerShell-rendered composite of the placeholder-mark vector (white flame on `#1A1A2E`) shipped 2026-08-07 to unblock licaon-kter's ask. **Removed 2026-08-19** with the rest of the store assets — user's plan is to ship a proper designer version. F-Droid's store page currently shows no icon; the on-device launcher (also placeholder-mark, QOL-4) is unaffected. | Render a 512×512 raster from whatever launcher SVG the QOL-4 design pass lands on — dark-mode-friendly, non-explicit, matches the on-device launcher so store + phone read consistent. Drop into `fastlane/metadata/android/en-US/images/icon.png`. | **LOW.** Design-bound; pairs with QOL-4 + STORE-1. |
...
| **LIC-1** | ✅ **DONE (2026-08-19).** **SPDX per-file license headers** | Was: 0 of 99 main files carried an SPDX identifier. | Added `// SPDX-License-Identifier: GPL-3.0-or-later` as line 1 of every `.kt` under `app/src/` (**166 files** — main + test + androidTest — the file count grew past 99 through the v0.4/v0.5 line's Photos/backup/audit-fix work). Scripted, idempotent (skips already-headered files). Reinforces the "verifiably open" pitch that IS this app and gives license scanners the machine-readable declaration they expect. | Done. |
...
| **SEC-2** | **Optional extra gate on the Photos tab** *(added 2026-07-12; blur tier ✅ 2026-07-12)* | ✅ **Blur tier DONE** (user chose the blur/reveal tier only). A **Blur photos until tapped** toggle in Settings → Gallery (`GalleryPreferences`, off by default); when on, the Photos tab renders behind `Modifier.blur` + a "Show photos" cover and re-arms on tab entry **after a 30s grace** (a process-scoped `GalleryRevealState` singleton tracks last-active-in-Photos, so a quick tab switch away and back stays revealed). **Still open:** the **re-auth tier** (biometric/PIN presence check to enter Photos) — the stronger opt-in, using the existing `BiometricPrompt`/device-credential path; not built per the user's scope choice. | The Photos tab is the **most concentrated** sensitive surface — every intimate photo in one place — so it's the highest-value spot for an **optional** extra layer against the "already-unlocked, phone glanced-at / handed over" threat. Two tiers, both **opt-in** via a `tryst_general`-style setting, **default off** (the app lock already covers the primary threat): **(1) require re-auth** to open Photos — reuse the existing `BiometricPrompt` / auth-gated Keystore path (**no new crypto**); **(2) lighter "reveal" gate** — blur thumbnails behind a one-tap "Show photos" interstitial so the gallery never renders by accident. Add a short grace period so it doesn't re-prompt on every tab switch. No schema change. See the 2026-07-12 discussion below. | **LOW–MED.** |
...
**Recommended near-term ordering (2026-06-28 review; updated 2026-08-19):** ~~CI-1~~ ✅ + ~~DOC-1~~ ✅ + ~~DOC-2~~ ✅ + ~~LIC-1~~ ✅ done → **QOL-5** as the next user-facing feature. **TEST-1 / SEC-1 / PERF-1** as capacity allows; **REFAC-1** opportunistically. *Housekeeping backlog is caught up; next work is user-facing (QOL-5) or the design bundle (QOL-4 + STORE-1 + STORE-2 + D-54, best shipped together).*
...
- **New dependency:** every item on this file stays dependency-free. Video (MED-1 / GAL-2)
  is the only known feature that would add a runtime dep (Media3 / ExoPlayer); it lives in
  [ROADMAP_VIDEO.md](ROADMAP_VIDEO.md) with its own dep-vet checklist.
...
### Discussion — an extra gate on the Photos tab (2026-07-12, feeds SEC-2)
Worth doing, but keep it **optional and off by default**. The reasoning:
- **The primary threat is already covered.** The app opens behind the PIN/biometric lock, auto-locks on
  background, and `FLAG_SECURE` blocks screenshots + the app-switcher preview. So a second gate isn't
  about the "phone is lost/stolen" case — that's handled.
- **What it *does* add** is protection for the realistic "phone is unlocked in someone's hand" moment:
  showing a friend a photo, passing the phone across a table, a partner glancing over. The gallery is the
  single worst surface for that because it's *everything at once* — which is exactly why it's the one
  place an extra gate earns its friction.
- **Two intensities**, so the user picks their comfort: (1) **re-auth** (biometric/PIN) to enter Photos —
  strongest, reuses the lock we already have, no new crypto; (2) a **"reveal" gate** — thumbnails blurred
  behind a one-tap "Show photos", so nothing renders by accident but there's no auth friction. (2) is the
  gentler default if they want it on.
- **Friction is the whole risk.** Re-prompting on every tab tap would get abandoned fast, so a short
  grace period (e.g. don't re-ask within N seconds/minutes of the last unlock) is essential.
- **Consistency caveat:** why gate only Photos and not a specific tryst's detail? Because the gallery is
  the *concentration* — one tap, all of it. Per-tryst gating is diminishing returns. If they want it
  broader later, the same setting could extend to the whole app-resume, but that's really just the
  existing auto-lock timeout turned down.
- **My recommendation:** ship tier (2), the reveal/blur gate, first — it delivers most of the value with
  none of the annoyance — and offer tier (1) re-auth as the stricter opt-in for anyone who wants it.
