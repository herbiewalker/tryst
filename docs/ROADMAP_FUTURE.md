...
|----|------|---------------|----------|-------------------|
| **CI-1** | **Run instrumented tests in CI (the data-safety net)** | CI runs `assembleDebug testDebugUnitTest lint checkNoNetworkDebug` + a banned-SDK scan. The **`MigrationTest`, `BackupRestoreRegressionTest`, `DbEncryptionTest`, crypto/vault tests run only locally** on the maintainer's machine. | Add an emulator job (`reactivecircus/android-emulator-runner`) running `connectedDebugAndroidTest` — at minimum `MigrationTest` + the backup regression tests — on push. A botched migration is the one bug class that silently corrupts real user data; CI is currently blind to it. | **HIGH.** Medium effort (emulator job is slow but well-trodden). **The single most valuable item here.** |
...
| **CLEAN-1** | **Delete scaffold tests** | `app/src/test/.../ExampleUnitTest.kt` (asserts `2+2==4`) and `app/src/androidTest/.../ExampleInstrumentedTest.kt` are Android Studio template leftovers. | Remove both. | **LOW.** Trivial. |
...
**Recommended near-term ordering (2026-06-28 review):** **CI-1 + STORE-1** first (protect data integrity + adoption; neither touches app logic) → fold **CLEAN-1 / LIC-1** (DOC-1 already done) into one tidy-up commit → then **QOL-5** as the next user-facing feature. **TEST-1 / SEC-1 / PERF-1** as capacity allows; **REFAC-1** opportunistically.
