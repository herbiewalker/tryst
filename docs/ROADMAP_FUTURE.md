...
| **SEC-2** | **Optional extra gate on the Photos tab** *(added 2026-07-12; blur tier ✅ 2026-07-12; re-auth tier ✅ 2026-08-31)* | ✅ **Both tiers DONE.** Blur tier (2026-07-12): "Blur photos until tapped" toggle + 30s configurable grace via `GalleryRevealState`. **Re-auth tier (2026-08-31, v0.5.7):** new **Require re-auth for Photos** toggle in Settings → Gallery (`GalleryPreferences.requireReauthForPhotos`, off by default). Uses `BiometricPromptHelper.confirmPresence` — `BIOMETRIC_STRONG or DEVICE_CREDENTIAL`, no CryptoObject, so it's a **presence check with no new crypto** (works whether biometrics or only the screen lock is set). Grace window mirrors the blur tier's (Immediate / 15s / 30s / 1 min / 5 min, default 30s). Toggle hides on devices with no biometric AND no screen lock. New `GalleryAuthState` singleton parallels `GalleryRevealState`; `GalleryScreen.AuthGate` cover sits above `BlurGate`. See D-58. | The Photos tab is the **most concentrated** sensitive surface — every intimate photo in one place — so it's the highest-value spot for an **optional** extra layer against the "already-unlocked, phone glanced-at / handed over" threat. Both tiers ship independently opt-in. |
