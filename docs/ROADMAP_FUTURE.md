...
| **GAL-1a** | **Avatars in the gallery** *(added 2026-07-12; partner-header avatars ✅ 2026-07-12)* | ✅ **Partner-section headers now show the partner's avatar + name** (from `PartnerEntity.photoMediaId`, decoded via `PartnerRepository.openPhoto`; initial-on-tint fallback when no photo). **Still open:** surfacing avatars/profile photos as *browsable gallery items* in their own right (they carry no date and aren't filterable, so they'd need separate handling from encounter photos). |
| **GAL-1b** | **Justified/mosaic tiles** *(added 2026-07-12)* | The date layout ships as a square grid for v1; a true aspect-preserving justified/mosaic row layout (Google Photos style) needs per-photo aspect ratios (decode bounds first). Pure presentation change over the existing pipeline. |
