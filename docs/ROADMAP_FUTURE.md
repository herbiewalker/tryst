...
> **Status: ACTIVE roadmap (last reviewed 2026-09-07).** Originally a raw idea dump (2026-06-14).
...
> **v0.5.7** SEC-2 tier 2 Photos re-auth, **v0.5.8** QOL-4 T-heart launcher icon + STORE-2
...
> model expansion — deferred to a v0.6 design pass). Next line is v0.6.x — candidates are QOL-5
> (settings-in-backup), BKP-1 (automatic local backups), STORE-1 (designer screenshots).
...
| **QOL-4** | **Updated icon set** | ✅ **Launcher done (2026-09-06, v0.5.8).** Replaced the M1 placeholder-flame with a designed **T-heart** glyph on a pink-to-purple gradient. Full adaptive-icon setup: foreground PNG at 5 densities + background-colour fallback (`#56174D`), padded to `innerRatio=0.78` so the mark lands in the launcher safe zone; outer padding is a synthesised vertical linear gradient sampled from the source's top-center → bottom-center pink-purple, so the join with the icon's own bleed is seamless. F-Droid store `icon.png` reshipped from the same source (**STORE-2 ✅**). **Still pending:** themed/monochrome variant for Material You; per-act vector icon refresh (drawable-swap-only seam via `ActVisuals`). |
...
| **STORE-2** | **F-Droid `icon.png`** *(added 2026-08-07 per F-Droid issue #3; placeholder PULLED 2026-08-19)* | ✅ **DONE (2026-09-06, v0.5.8).** 512×512 T-heart on pink-to-purple gradient, rendered from the same source as the QOL-4 launcher so store + phone icons read consistent. Shipped to `fastlane/metadata/android/en-US/images/icon.png`. | Done. | Done. |
...
**Recommended near-term ordering (updated 2026-09-07):** ~~CI-1~~ ✅ + ~~DOC-1~~ ✅ + ~~DOC-2~~ ✅ + ~~LIC-1~~ ✅ + ~~QOL-4 (launcher)~~ ✅ + ~~STORE-2~~ ✅ done → **QOL-5** (settings-in-backup) or **BKP-1** (automatic local backups) as the next user-facing feature for the v0.6.x line. **STORE-1** (designer screenshots) still pending — pairs naturally with any v0.6.x release. **TEST-1 / SEC-1 / PERF-1** as capacity allows; **REFAC-1** opportunistically; **QOL-4 themed/monochrome variant + act-icon refresh** to close out that theme.
