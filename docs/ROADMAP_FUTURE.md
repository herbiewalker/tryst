...
| ID | Item | Notes |
|----|------|-------|
| **CAP-1** | **Per-photo captions** | A short note on an individual photo (the tryst note describes the event, not the picture). Additive `media.caption` column (schema bump + migration + test); searchable by folding it into the SRCH-1 index. Display in the viewer + as a tile long-caption. |
