...
| **STORE-1** | ✅ **DONE (2026-07-02b, revised 2026-08-07 per F-Droid issue #3).** **F-Droid listing screenshots** | Was: no `images/phoneScreenshots/` — empty gallery. | Added **6 dark-theme phone screenshots** (Trysts list, calendar heatmap, Insights overview + breakdowns, Achievements, Partners) under `fastlane/.../images/phoneScreenshots/`, captured via the `FLAG_SECURE`-off procedure over a **scrubbed synthetic dataset** (explicit act/kink tokens remapped to v10 built-ins; helpers `tools/dataset/Dump.java`/`Extract.java`). **2026-08-07 revision:** licaon-kter (F-Droid) opened issue #3 flagging `04_insights_breakdowns` + `07_manage_categories` as stepping over the all-ages store-page policy — both removed; README screenshot table trimmed to match. Store listing now shows 5 screenshots. Optional `featureGraphic` still not added. | Done. |
| **STORE-2** | **Nicer F-Droid `icon.png`** *(added 2026-08-07 per F-Droid issue #3)* | Current `fastlane/metadata/android/en-US/images/icon.png` is a 512×512 PowerShell-rendered composite (white vector flame on `#1A1A2E`) shipped 2026-08-07 to unblock licaon-kter's ask. Functional but not artisanal — same "placeholder mark" note that lives on the launcher vector. | Replace with a proper designer-made 512×512 PNG that matches the eventual **QOL-4** launcher-icon refresh (adaptive + Material You monochrome). Should stay dark-mode-friendly, non-explicit (F-Droid all-ages), and echo whatever mark QOL-4 lands on so the store icon + on-device launcher are consistent. Drop-in file swap under the same path — no code change. Pairs with QOL-4; ideally shipped together. | **LOW.** Design-bound. |
...
**Recommended near-term ordering (2026-06-28 review; updated 2026-08-19):** ~~CI-1~~ ✅ + ~~STORE-1~~ ✅ + ~~DOC-1~~ ✅ + ~~DOC-2~~ ✅ + ~~LIC-1~~ ✅ done → **QOL-5** as the next user-facing feature. **TEST-1 / SEC-1 / PERF-1** as capacity allows; **REFAC-1** opportunistically. *Housekeeping backlog is now caught up; next work is user-facing (QOL-5) or design (STORE-2 / QOL-4 / D-54).*
