...
| **EDIT-1** | **In-app photo edit (crop / rotate)** | Decrypt → transform → re-encrypt in place (or as a new blob). Rotate/crop need **no new dependency** (`Bitmap`/`Matrix`); everything stays in-memory, `FLAG_SECURE` covers the editor. Watch orientation/EXIF (pairs with META-1's strip-on-export note). |
