...
**Recommended near-term ordering (2026-06-28 review; updated 2026-07-02b):** ~~CI-1~~ ✅ + ~~STORE-1~~ ✅ done → **DOC-2** (docs modernization) + **LIC-1** (SPDX headers) tidy-up (DOC-1 ✅) → then **QOL-5** as the next user-facing feature. **TEST-1 / SEC-1 / PERF-1** as capacity allows; **REFAC-1** opportunistically. *Remaining quick wins: DOC-2 (docs modernization), LIC-1 (SPDX headers).*
