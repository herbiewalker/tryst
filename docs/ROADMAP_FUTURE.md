...
| **QOL-7** | **Promote remaining pop-out dialogs to their own pages** *(added 2026-07-12)* | Several editors/config surfaces are still `AlertDialog` pop-outs, while most settings sub-surfaces are already full-screen routes (`ProfileScreen`, `ChangePinScreen`, `CatalogManageScreen`, `GallerySettingsScreen`, Reset, About, What's-new). | **Audit** the remaining dialog pop-outs and convert the substantial ones (multi-field forms / management surfaces) to dedicated routes for a consistent, roomier feel; leave genuine one-shot pickers (date/time/range, confirm gates) as dialogs. Known headline is **QOL-3** (partner editor → page); the audit enumerates the rest. Carry the discard-changes guard (D-33) where a form gains a route. Pure UI/navigation; no schema change. |
