...
> (manual/fingering), `ic_act_anal`, `ic_act_rimming`, `ic_act_sixtynine`, `ic_act_massage`,
> `ic_act_foot`, `ic_act_squirt`, `ic_act_pegging`, `ic_act_prostate`, `ic_act_fisting`,
> `ic_act_scissoring`, `ic_act_dp` (double penetration), `ic_act_a2m`, plus a neutral **`ic_act_custom`**
> fallback (a generic "spark/heart/asterisk" used for any user-created act).
>
...
- **Current style** (the bar to match): `ic_act_*.xml` — 24dp viewport, `strokeColor #000000`,
  ...
  `ui/common/PracticeVisuals.kt`. Because tint happens in code, **swapping the drawables needs no code
  change** (QOL-4 is a pure asset swap).
...
- **Acceptance for QOL-4:** new act set drops into `res/drawable/` under the same names; new adaptive
  ...
  the act picker (`PracticeVisuals`).
